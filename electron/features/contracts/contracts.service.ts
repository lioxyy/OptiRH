...
// Fetch all contracts (restricted by role)
...
export async function createContract(data: {
  ...
  return prisma.$transaction(async (tx) => {
    // 2. Set any existing active contracts for this employee to 'Expired'
...
// Safe Contract Soft-Archiving (Agent/Admin)
export async function archiveContract(contractId: number, actorId: number) {
  ...
  return prisma.$transaction(async (tx) => {
    const updated = await tx.contract.update({
      where: { id_contract: contractId },
      data: { status: 'Archived' } // Soft transition instead of hard-delete
    })
