...
export async function getFormations() {
  return prisma.formation.findMany({ include: { instructor: { select: { id_emp: true, name: true } } } })
}

export async function getFormationById(id: number) {
  const f = await prisma.formation.findUnique({ where: { id_formation: id }, include: { instructor: { select: { id_emp: true, name: true } } } })
